...
export default class ChooseOrientationPage extends React.Component {
  constructor(props) {
    super(props);
    var data =  [
                  {name:"Portrait"},
  ...
  componentDidMount () 
  {
    console.log("ChooseOrientationPage componendDidMount");
  }
  ...
  chooseOrientation = (orientation) => 
  {
    ...
    {
      ToastExample.savePreferenceOrientation(orientation);
    }
    
    this.props.navigation.goBack();
    this.props.navigation.state.params.onSelect({ orientation: orientation });
  }

  setOrientation = (orientation) => 
  {
    var connectBrotherPrinter = NativeModules.ConnectBrotherPrinter;
    connectBrotherPrinter.setOrientation(orientation);
  };
      ...
      <View>
        <FlatList          
          ...
          renderItem={({ item }) => ( 
              ...
              <TouchableHighlight 
                underlayColor={'white'} activeOpacity={1} onPress={()=>{this.chooseOrientation(item.name)}} >                                                             
                <View>
