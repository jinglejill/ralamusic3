...
export default class ChooseAutoCutPage extends React.Component {
  constructor(props) {
    super(props);
    var data =  [
                  {name:"YES"},
  ...
  chooseAutoCut = (autoCut) => 
  {
    if(Platform.OS == 'ios')
    {      
      this.setAutoCut(autoCut);
    }
    else
    {
      ToastExample.savePreferenceAutoCut(autoCut);  
    }
    
    this.props.navigation.goBack();
    this.props.navigation.state.params.onSelect({ autoCut: autoCut });
  }

  setAutoCut = (autoCut) => 
  {
    var connectBrotherPrinter = NativeModules.ConnectBrotherPrinter;
    connectBrotherPrinter.setAutoCut(autoCut);
  };
      ...
      <View>
        <FlatList          
          ...
          renderItem={({ item }) => ( 
              ...
              <TouchableHighlight 
                underlayColor={'white'} activeOpacity={1} onPress={()=>{this.chooseAutoCut(item.name)}} >                                                             
                <View>
