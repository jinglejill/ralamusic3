...
export default class OrderGroupListPage extends React.Component 
{
  ...
  { 
    ...
    .then(res => {      
      this.setState({
        data: page === 1 ? res.orderDeliveryGroupList : [...this.state.data, ...res.orderDeliveryGroupList],
  ...
  {    
    ...
    const url = this.state.apiPath + 'SAIMOrderDeliveryGroupInsert.php';          
    ...
    .then(res => {      
      ...
      this.setState({orderDeliveryGroupID:res.orderDeliveryGroupID});
      this.props.navigation.navigate('OrderNoScan',
      {
        orderDeliveryGroupID: res.orderDeliveryGroupID,
  ...
  {
    this.props.navigation.navigate('OrderDetailList',
    {
      ...
      <View style={{flex:1}} onStartShouldSetResponder={this.containerTouched.bind(this)}>                       
        <FlatList   
          ...
          renderItem={({ item }) => 
            <SwipeRow               
              rightOpenValue={-100}
              ref={ref => {this.setSwipeRowRef(ref,item.OrderDeliveryGroupID)}}
            >
                  ...
                  <View style={{ flex: 1}}>
                    <TouchableHighlight
                      ...
                      <View style={{display:'flex',flexDirection:'row',height:40,alignItems:'center'}}>
                        <Text style={styles.title2}>#{item.OrderDeliveryGroupID}</Text>
                        <Text style={styles.title4}> (qty. {item.OrderCount})</Text>
                        <View style={{flex: 1,alignItems:'flex-end'}}>
                          <Text style={styles.title3}>{item.CheckDate}</Text>
                        </View>
                      </View>
                    </TouchableHighlight>
