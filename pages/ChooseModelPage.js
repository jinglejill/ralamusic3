...
export default class ChooseModelPage extends React.Component {
  constructor(props) {
    super(props);
    var data =  [
                  {name:"QL-720NW"},
  ...
  chooseModel = (model) => 
  {
    ...
    {
      ToastExample.savePreferenceModel(model);
    }
    
    
    this.props.navigation.goBack();
    this.props.navigation.state.params.onSelect({ model: model });
  }

  setModel = (model) => 
  {
    var connectBrotherPrinter = NativeModules.ConnectBrotherPrinter;
    connectBrotherPrinter.setModel(model);
  };
      ...
      <View>
        <FlatList          
          ...
          renderItem={({ item }) => ( 
              ...
              <TouchableHighlight 
                underlayColor={'white'} activeOpacity={1} onPress={()=>{this.chooseModel(item.name)}} >                                                             
                <View>
