...
export default class ScanInPage extends React.Component {
  constructor(props) {
    super(props);
  ...
  barcodeDetected(barcode)
  {
    ...
    const url =  this.state.apiPath + 'SAIMProductScanInUpdate2.php';    
    ...
    .then(res => {
      if(res.success == true)
      {
        var scanQuantity = res.quantityIn;         
        if(this.state.previousScanSku == res.sku)
        {
          scanQuantity = parseInt(this.state.scanQuantity) + parseInt(res.quantityIn);        
        }
        else
        {
